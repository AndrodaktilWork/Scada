<?php
// Десктоп хендлър — връща по подразбиране към index.php
if (!headers_sent()) {
...
try {
    ...
    // безопасен редирект
    $redir = $_POST['redirect'] ?? ($_SERVER['HTTP_REFERER'] ?? 'index.php');
    if (preg_match('/^https?:/i',$redir) || !preg_match('/^[A-Za-z0-9_\-\/\.\?\=&%]+$/',$redir)) $redir='index.php';
